...
// CONSTANTS
const Hysteresis = 60000; // Wait time in milliseconds before starting fetch favicon after add-on start.
const Cadence = 200; // Wait time in milliseconds between a request completion and start next one in queue
...
// bnId = Id of BookmarkNode. uri is a String
...
// BN is a BookmarkNode. url is a String
const MyInitFF56 = {
  credentials: "omit",
  mode: "cors",
  redirect: "follow"
...
// bnId = Id of BookmarkNode. url = URL string. enableCookies = a Boolean to "omit" (false) or "include" (true)
  ...
  fetch(url, myInit).then(
    function (response) { // This is a Response object
      ...
      if (response.status == 200) { // If ok, get contents as blob
        response.blob().then(function (blob) { // blob is a Blob ..
...
const PatternFavicon1 = /rel\s*=\s*\"shortcut icon\"/;
  ...
  fetch(bnUrl, myInit).then(
    function (response) { // response is a Response object
      ...
      //if (response.status == 200) { // If ok, get contents as a string
        response.text().then(function (text) { // text is a USVString returned by Response.text()
          ...
          if (faviconUrl == null) { // If not found, try the old root/favicon.ico method
            let urlObj = new URL (response.url); // Use the URL of response in case we were redirected ..
          ...
          if (faviconUrl.startsWith("//")) { // Scheme (protocol, e.g. https:) relative URL
          ...
          else { // Just relative URL ... Verify if there is a <base href=..> tag
            let baseUrl = retrieveBaseUrl(text);
            ...
            if (baseUrl != null) {
              ...
              // Need to get back to closest '/' from end of path, removing all ?xxx=yy and other stuff
...
function handleRequest() {
  hysteresis = Cadence; // Hysteresis is a one shot only thing, for the first fetch of massive load, so now reset to default.
  ...
  if (bnId != undefined) { // Protect against too many redispatch events
    ...
    else if (action == "icon") { // We already got the favicon URL (e.g. tab refresh) so simply get it.
